...
Guidance specific to agent-driven development on this repo. Read the
companion [DEVELOPMENT.md](DEVELOPMENT.md) for the project's general
dev-process content -- this file layers agent-specific behaviors on
top.
...
bodies, docs) is ASCII only -- see `DEVELOPMENT.md` for the rule
and the slip-replacements list. Replies in the chat itself are
...
The Doc-sync rule in `DEVELOPMENT.md` is mandatory for every
...
`DEVELOPMENT.md`'s "Commit messages" section is the canonical rule
list. A few patterns recur in agent-authored messages despite being
on the do-NOT list, so flagging them again here:
...
`# 2. Validate`, ...) are forbidden by `DEVELOPMENT.md`'s
...
`DEVELOPMENT.md`'s "Comments" subsection; the agent-specific
failure mode is repeating the commit-message rationale inside the
source.
...
include" list under Commit-message hygiene in `DEVELOPMENT.md`)
are illustrative, not exhaustive. They're samples of patterns to
recognise, not authoritative enumerations -- when a similar-but-
not-included entry shows up, the list doesn't need to be extended
