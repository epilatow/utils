...
- `scripts/` -- developer/build tooling, not user-facing utilities.
  ...
  roff man page and a GitHub-browsable GFM doc per documented utility, plus
  the repo `README.md`; see Testing). A utility opts into docs by exposing a
  module-level `MAN_SPEC` (a `common.docspec.ManSpec`); render-docs discovers
  them, so documenting a utility touches only that utility, not this script.
  render-docs holds only formatting + parser introspection -- the per-utility
  content lives with the utility (overview text, an argument's `extended_help`
  / `common=True` tag via `common.argparse_ext.add_argument_ext`, the
  `MAN_SPEC` fields). `scripts/pandoc` manages the pinned pandoc the man-page
  gate needs (`pandoc install` fetches it into `.tools/`; see Testing), with
  the pin in `scripts/pandoc-pin.json`. Like `bin/` entries these are PEP 723
  executables with a `src/<module>.py` alias for import + lint coverage.
- `README.md` -- the repo's front page, generated by `scripts/render-docs`: an
  authored preamble, the documented utilities (linked to their `docs/` page),
  and the undocumented `bin/` utilities (each blurbed from its `--help`). A
  generated build artifact, never hand-edited; the authored preamble (title,
  intro, Installation) lives in `scripts/readme-intro.md`, which is the only
  hand-edited part. The freshness gate (see Testing) fails if it drifts.
...
- The test suite owns ruff and mypy enforcement -- a green run is the gate for
  "ready to commit".
- `tests/test_render_docs.py` is a freshness gate: for each documented utility
  it re-renders the roff man page (e.g. `share/man/man1/crony.1`) and the GFM
  doc (e.g. `docs/crony.md`) from the argparse parser, and re-renders the repo
  `README.md`, failing if any checked-in file differs. The GFM docs and the
  README are built in pure Python (plus mdformat), so their comparison needs
  no pandoc (the README does run each undocumented utility's `--help`); the
  roff comparison shells out to pandoc, so **pandoc is a required dev/CI
  dependency** -- a missing pinned pandoc fails the gate rather than skipping
  it. pandoc's roff output varies by version, so a single pandoc is pinned in
  **`scripts/pandoc-pin.json`** (the version plus the sha256 of each platform
  ...
  artifacts -- the man page and GFM doc for a documented utility, and the
