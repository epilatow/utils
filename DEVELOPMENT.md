...
- `scripts/` -- developer/build tooling, not user-facing utilities.
  ...
  the repo `README.md`; see Testing); each documented utility is a `ManSpec`,
  and adding one is appending to the script's `_SPECS`. `scripts/pandoc`
  manages the pinned pandoc the man-page gate needs (`pandoc install` fetches
  it into `.tools/`; see Testing), with the pin in `scripts/pandoc-pin.json`.
  Like `bin/` entries these are PEP 723 executables with a `src/<module>.py`
  alias for import + lint coverage.
