...
import darwin_tz_watchdog as dtw  # noqa: E402
...
def test_main_noops_on_non_darwin(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # Off darwin the watchdog parses its args (so --help and validation
    # behave the same everywhere) but then no-ops without restarting.
    ...
    assert dtw.main([]) == dtw.ExitCode.SUCCESS
    sentinel.assert_not_called()


def test_main_help_works_off_darwin(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    # --help must render on every platform: the parser is shared by the
    # cross-platform no-op path and feeds the generated docs, so its
    # output must not depend on running under darwin.
    monkeypatch.setattr(dtw.platform, "system", lambda: "Linux")
