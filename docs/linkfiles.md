...
target (`bashrc` becomes `.bashrc`). A source entry whose top-level name
begins with a dot (like .git or .github) is skipped, though dot-names below
the top level are linked as ordinary content. Files matched by an ignore file
